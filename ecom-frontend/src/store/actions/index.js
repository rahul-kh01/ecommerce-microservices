...
export const authenticateSignInUser 
    = (sendData, toast, reset, navigate, setLoader) => async (dispatch) => {
        try {
            ...
            // Extract token from response - backend returns 'jwtToken'
...
export const getOrdersForDashboard = (queryString, roles) => async (dispatch) => {
    try {
        dispatch({ type: "IS_FETCHING" });
        ...
        if (roles?.isAdmin) {
            ...
            endpoint = "/users/orders";  // Regular users
        }
...
export const updateOrderStatusFromDashboard =
     (orderId, orderStatus, toast, setLoader, isAdmin, isSeller) => async (dispatch, getState) => {
    try {
        setLoader(true);
        ...
        // Refetch orders with proper parameters
        const queryString = "pageNumber=0";
        const roles = { isAdmin, isSeller };
        await dispatch(getOrdersForDashboard(queryString, roles));
